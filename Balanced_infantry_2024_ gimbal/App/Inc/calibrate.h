...
#define imu_start_buzzer()          TIMPWMParamSet(&htim4,TIM_CHANNEL_3,95, 10000)    
//when gimbal is calibrating ,buzzer set frequency and strength.当云台在校准,蜂鸣器的设置频率和强度
#define gimbal_start_buzzer()       TIMPWMParamSet(&htim4,TIM_CHANNEL_3,31, 19999)    
#define cali_buzzer_off()           TIMPWMParamReset(&htim4,TIM_CHANNEL_3)      //buzzer off，关闭蜂鸣器


//get stm32 chip temperature, to calc imu control temperature.获取stm32片内温度，计算imu的控制温度
#define cali_get_mcu_temperature()  GetTemprate()      
...
#define get_remote_ctrl_point_cali()        GetRemoteControlPoint()  //get the remote control point，获取遥控器指针
#define gyro_cali_disable_control()         USARTUnable(&huart3)                 //when imu is calibrating, disable the remote control.当imu在校准时候,失能遥控器
#define gyro_cali_enable_control()          USARTRestart(&huart3,&hdma_usart3_rx)
