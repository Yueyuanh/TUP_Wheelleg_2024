 ...
 *	旋转矩阵调试方法：
 *	1、如图建立坐标系x,y,z轴
 *  2、对比原C板Bmi088坐标系和机器人坐标系（x轴：后方；y轴：右方；z轴：上方）的区别
 *	3、根据以上基础数学知识推出应该如何旋转顺序可将原坐标系转化为机器人坐标系
 *	注意：旋转θ的正方向为顺时针（按道理来说是逆时针，但在此处若按照理论去观察数据为顺时针，不符合右手定则）
 ******************************************************************************
 */
#include "imu.h"
...
#include "calibrate.h"
/*
	INS_Accel：负值时与重力同方向为正方向
	INS_Accel、INS_gyro：表示原bmi088坐标系转化后的信息
	原BMI088坐标系朝向R标为X轴正方向；朝向SWD线接口为Y轴正方向；朝向上方为z轴
	经过旋转矩阵后：X轴朝向机器人后方、Y轴朝向机器人右侧、Z轴朝向机器人上方
	Gyro和欧拉角通过右手定则判断
*/
...
void ImuInit()
{
   ...
   imu_temp_pid.Init(PID_POSITION, imu_temp_PID, TEMPERATURE_PID_MAX_OUT, TEMPERATURE_PID_MAX_IOUT);
...
//  get_angle(INS_quat, ins_msg.imu_angle + INS_YAW_ADDRESS_OFFSET, ins_msg.imu_angle + INS_PITCH_ADDRESS_OFFSET, ins_msg.imu_angle + INS_ROLL_ADDRESS_OFFSET);
...
static void imu_temp_control(fp32 temp)
{
    ...
    if (first_temperate)
    {
        imu_temp_pid.Calc(temp, 35);  //温度需改（应该是校准后MCU温度，这里直接实时，不进行赋值了）--get_control_temperature()
        if (imu_temp_pid.out < 0.0f)
        {
            imu_temp_pid.out = 0.0f;
        }
        tempPWM = (uint16_t)imu_temp_pid.out;
				TIMSetPWM(&htim10,TIM_CHANNEL_1,tempPWM);
    }
    else
    {
        //在没有达到设置的温度，一直最大功率加热
        //in beginning, max power
        if (temp > GetTemprate()+10)   //温度需改（应该是校准后MCU温度，这里直接实时，不进行赋值了）--get_control_temperature()
        {
            temp_constant_time++;
            if (temp_constant_time > 200)
            {
				...
				TIMSetPWM(&htim10,TIM_CHANNEL_1,MPU6500_TEMP_PWM_MAX - 1);
    }
}
